...
_WARNING_CODES = frozenset({"UNMATCHED_END_TAG", "IMPLICITLY_CLOSED_TAG", "UNCLOSED_TAG"})
...
def summarize_diagnostics(files: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    affected_files: dict[str, set[str]] = {"parser_error": set(), "parser_warning": set(), "unsupported": set(), "unclassified": set()}
    total = 0
    ...
    return {
        ...
        "by_language": {language: dict(sorted(counter.items())) for language, counter in sorted(by_language.items())},
        "affected_files": {category: sorted(paths) for category, paths in sorted(affected_files.items()) if paths},
    }
